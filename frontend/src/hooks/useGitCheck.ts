...
export function useGitCheck(options: UseGitCheckOptions = {}) {
  ...
  // 加载 Python 版本列表（先自动发现再读取）
  const loadPythonVersions = async () => {
    setIsLoadingPython(true);
    setPythonError("");
    environmentLogger.info("自动发现并加载 Python 版本列表");
  ...
  useEffect(() => {
    checkGitEnvironment();
    loadPythonVersions();
    // checkGitEnvironment/loadPythonVersions 未记忆化，加入依赖会每次渲染重复请求，故仅挂载时执行一次

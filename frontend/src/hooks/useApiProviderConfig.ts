...
export function useApiProviderConfig() {
  ...
  const removeProvider = async (index: number) => {
    ...
    if (provider.id) {
      try {
        ...
        environmentLogger.success("供应商已删除", {
          name: provider.name,
          id: provider.id,
        });
      } catch (error) {
  ...
  const validateApiKey = async (index: number, provider: ApiProvider) => {
    ...
    try {
      // 首先保存配置（如果还没有ID）
      if (!provider.id) {
        environmentLogger.info("供应商无ID，先保存以获取ID", {
          provider: provider.name,
        });
      ...
      // 测试连接 — 远程获取模型列表
      const data = await tauriInvoke<{ models: string[]; modelsCount: number }>(
  ...
  const autoSaveProviders = async (providersToSave: ApiProvider[]) => {
    environmentLogger.info("自动保存 API 供应商配置", {
      count: providersToSave.length,
    });

    // 过滤出有效的供应商（必填字段都已填写）
    const validProviders = providersToSave.filter(
      (p) => p.name.trim() && p.base_url.trim() && p.api_key.trim(),
    );
    ...
    environmentLogger.info("准备保存的供应商", {
      validCount: validProviders.length,
      providers: validProviders.map((p) => p.name),
    });
    ...
    try {
      ...
      setSaveStatus({ success: true, message: "[成功] 已自动保存" });

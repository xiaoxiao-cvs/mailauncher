...
export function useWebSocket(
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      try {
        // 监听日志事件
        const unLog = await transport.listen<string>(
          downloadLogEvent(taskId),
          (message) => {
            if (cancelled) return;
        ...
        // 监听状态事件
        const unStatus = await transport.listen<string>(
          downloadStatusEvent(taskId),
          (status) => {
            if (cancelled) return;
            const now = new Date().toISOString();
        ...
        // 监听结构化进度事件
        const unProgress = await transport.listen<DownloadProgressPayload>(
          downloadProgressEvent(taskId),
          (payload) => {
            if (cancelled) return;
            const now = new Date().toISOString();
            const { percentage, message, status } = payload;

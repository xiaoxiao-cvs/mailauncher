...
export function useDownloadActions(params: UseDownloadActionsParams) {
  const { deploymentPath, instanceName, selectedMaibotVersion, pythonPath } =
    params;
  ...
  const updateItemStatus = useCallback(
    (
      itemId: string,
      status: Partial<
        Pick<DownloadItem, "status" | "progress" | "error" | "version">
      >,
    ) => {
  ...
  const downloadAll = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const task = await tauriInvoke<{ id: string }>("create_download_task", {
        data,
      });

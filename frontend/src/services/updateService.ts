 ...
 * 从 Rust 后端检查更新
 */
export async function checkUpdateFromBackend(
  channel: string = "main",
): Promise<UpdateCheckResponse | null> {
  try {
    const data = await tauriInvoke<UpdateCheckResponse>(
...
export async function getChannelVersions(
  ...
  try {
    const data = await tauriInvoke<ChannelVersionsResponse>(

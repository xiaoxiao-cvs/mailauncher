...
describe("versionApi", () => {
  ...
  describe("getInstanceComponentsVersion", () => {
    it("should invoke get_instance_components_version and return component list", async () => {
      ...
      expect(invoke).toHaveBeenCalledWith("get_instance_components_version", {
        instanceId: "inst_001",
      });
      expect(result).toHaveLength(2);
      expect(result[0].component).toBe("MaiBot");
      expect(result[0].version).toBe("1.2.0");
      expect(result[0].commit_hash).toBe("abc1234");
      expect(result[0].install_method).toBe("git");
      expect(result[0].installed_at).toBe("2026-03-20T10:00:00Z");
      expect(result[1].component).toBe("NapCat");
      expect(result[1].install_method).toBe("release");
    ...
    it("should propagate errors", async () => {
      ...
      await expect(
        getInstanceComponentsVersion("inst_missing"),
  ...
  describe("checkComponentUpdate", () => {
    it("should invoke check_component_update and return update check result", async () => {
      ...
      expect(result.current_version).toBe("1.2.0");
      expect(result.latest_version).toBe("1.3.0");
      expect(result.commits_behind).toBe(5);
      expect(result.update_notes).toBe("feat: new feature");
    });
  ...
  describe("updateComponent", () => {
    ...
    it("should propagate update errors", async () => {
      ...
      await expect(updateComponent("inst_001", "MaiBot")).rejects.toThrow(
        "Git pull failed",
      );
    });
  });
  ...
  describe("restoreBackup", () => {
    ...
    it("should propagate restore errors", async () => {
      ...
      await expect(restoreBackup("inst_001", "bak_bad")).rejects.toThrow(
        "Backup corrupted",
      );
    });
  });

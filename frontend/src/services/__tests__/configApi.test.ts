...
describe("configApi", () => {
  ...
  describe("getBotConfig", () => {
    it("should invoke get_toml_config with bot configType and wrap result", async () => {
      const mockData = {
        ...
        behavior: { reply_speed: 5 },
      };
      vi.mocked(invoke).mockResolvedValue({ data: mockData, comments: {} });

      const result = await getBotConfig("inst_001");

      expect(invoke).toHaveBeenCalledWith("get_toml_config_with_comments", {
        instanceId: "inst_001",
        configType: "bot",
    ...
    it("should pass null instanceId when omitted", async () => {
      vi.mocked(invoke).mockResolvedValue({ data: {}, comments: {} });
    ...
    it("should propagate errors from invoke", async () => {
      ...
      await expect(getBotConfig("inst_missing")).rejects.toThrow(
  ...
  describe("saveBotConfigRaw", () => {
    ...
    it("should propagate write errors", async () => {
      ...
      await expect(saveBotConfigRaw("content", "inst_001")).rejects.toThrow(
        "Permission denied",
      );
    });
  });
  ...
  describe("deleteBotConfigKey", () => {
    it("should invoke delete_toml_config_key and wrap result", async () => {
      ...
      const result = await deleteBotConfigKey(
        { key_path: "personality.name" },
        "inst_001",
      );
  ...
  describe("deleteModelConfigKey", () => {
    it("should invoke delete_toml_config_key for model config", async () => {
      ...
      const result = await deleteModelConfigKey(
        { key_path: "model.provider" },
        "inst_001",
      );
  ...
  describe("addModelConfigArrayItem", () => {
    it("should invoke add_toml_array_item for model config", async () => {
      const afterAdd = {
        endpoints: [{ url: "https://api.example.com", priority: 1 }],
      };
      vi.mocked(invoke).mockResolvedValue(afterAdd);

      const result = await addModelConfigArrayItem(
        {
          array_path: "endpoints",
  ...
  describe("saveAdapterConfigRaw", () => {
    ...
    it("should propagate write errors", async () => {
      ...
      await expect(saveAdapterConfigRaw("content", "inst_001")).rejects.toThrow(
        "Disk full",
      );
    });
  });

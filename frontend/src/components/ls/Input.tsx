...
/**
 * 文本输入 —— Living Surfaces 的凹陷式单行输入(native input 封装)。
 * ls-inset 观感:--ls-bg-2 底 + 发丝边 + 控件圆角,无投影,墨色文字。
 * 聚焦时边框与外环从发丝色弹簧过渡到生命色 --ls-life,给"激活"反馈;
 * 失焦归位。明暗双主题全靠 var(--ls-*) 自适配,占位色贴 --ls-ink-faint。
 * forwardRef 透传到底层 input,className 与全部原生 input 属性原样透传。
 */
  ...
  ({ className, style, onFocus, onBlur, ...props }, ref) => {
    ...
    return (
      <motion.input
        ...
        className={cn(
          "ls-num w-full px-3 py-2 text-sm outline-none placeholder:text-[color:var(--ls-ink-faint)]",
          className,
        )}
        style={{

...
export const TerminalComponent: React.FC<TerminalComponentProps> = ({
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      fontSize: 14,
      fontFamily:
        '"Cascadia Code", "Cascadia Mono", "JetBrains Mono", "Fira Code", Consolas, "DejaVu Sans Mono", "Courier New", monospace',
      // 确保字符宽度一致，有助于二维码正确显示
      letterSpacing: 0,
      lineHeight: 1,
      theme: {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        term.writeln("\x1b[1;32m[成功] 终端已连接\x1b[0m");
        term.writeln("");

        if (history && history.length > 0) {
          term.writeln(
            `\x1b[90m--- 历史日志 (${history.length} 条) ---\x1b[0m`,
          );
        ...
        if (isRunning && runtimeKind === "wsl2") {
          pollTimer = window.setInterval(async () => {
            ...
            try {
              const latestHistory = await tauriInvoke<string[]>(
        ...
        if (!cancelled) {
          term.writeln("");
          term.writeln(`\x1b[1;31m[失败] 终端连接失败: ${error}\x1b[0m`);

...
export const ConfigModal: React.FC<ConfigModalProps> = ({
  ...
  const [modelConfig, setModelConfig] = useState<ConfigWithComments | null>(
    null,
  );
  const [adapterConfig, setAdapterConfig] = useState<ConfigWithComments | null>(
    null,
  );
  ...
  // NapCat 状态
  const [napCatAccounts, setNapCatAccounts] = useState<
    Array<{ account: string; nickname: string }>
  >([]);
  const [selectedQQAccount, setSelectedQQAccount] = useState<string | null>(
    null,
  );
  const [originalQQAccount, setOriginalQQAccount] = useState<string | null>(
    null,
  );
      ...
      if (accountsResponse.success) {
        ...
        toast.success(
          accountsResponse.message ||
            `找到 ${accountsResponse.accounts.length} 个已登录账号`,
        );
      } else {
  ...
  useEffect(() => {
    ...
    // loadNapCatAccounts 未记忆化，加入依赖会每次渲染重建并触发重复加载，故仅按配置类型变化驱动
  ...
  useEffect(() => {
    ...
    // loadConfigs 未记忆化，加入依赖会每次渲染重建并触发重复加载，故仅在弹窗开启/实例切换时执行
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleSaveQQAccount = async () => {
    setSaving(true);
    try {
      await instanceApi.updateInstance(instanceId!, {
        qq_account: selectedQQAccount,
      } as any);

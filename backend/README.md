...
✅ **实例管理** - 完整的 MaiBot 实例生命周期管理
  - 创建、启动、停止、重启、删除实例
  - 多组件支持（MaiBot、NapCat、NapCat-ada）
  - 跨平台进程管理（Windows/Unix）
  - 实时状态监控
...
服务将在 `http://localhost:23456` 启动。

### 测试实例管理

```bash
python test_instance_management.py
```
...
├── API.md                           # API 端点文档
├── INSTANCE_MANAGEMENT_README.md    # 实例管理使用说明
├── test_instance_management.py      # 实例管理测试脚本
├── app/                             # 应用包
│   ├── __init__.py
│   ├── core/                        # 核心配置
│   │   ├── __init__.py

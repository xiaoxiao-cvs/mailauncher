...
> 废弃声明（DEPRECATED）：本文件描述的是旧版 Python / FastAPI HTTP API。当前架构已迁移为 Tauri IPC（前端通过 invoke 调用 Rust 命令），不再运行独立的 HTTP 后端。本文件仅作为迁移历史参考保留，不代表当前实现现状。
...
### 1. 代码组织
- [x] 遵循单一职责原则，每个模块只负责一个功能域
- [x] 使用 Pydantic 模型进行数据验证
- [x] 路由按功能分离到不同的路由器文件
- [x] 服务层与路由层分离，业务逻辑在 service 层实现

### 2. 错误处理
- [x] 统一的异常处理机制
- [x] 使用 HTTPException 返回规范的错误响应
- [x] 记录详细的错误日志
- [!] 建议：捕获更具体的异常类型，避免过于宽泛的 `except Exception`
...
- [x] 限制文件上传大小
- [x] 使用环境变量管理敏感配置
- [!] 建议：生产环境限制 CORS 的 `ALLOWED_ORIGINS`，不要使用 `*`
- [!] 建议：添加 API 限流机制

### 4. 性能优化
- [x] 使用异步处理（async/await）
- [x] 合理使用缓存（Python 版本、Git 信息等）
- [x] 后台任务处理耗时操作
- [!] 建议：为大列表查询添加分页功能
- [!] 建议：优化数据库查询，避免 N+1 问题

...
- `PathMapper` 已作为独立抽象落地，用于统一 local / guest 工作区和组件目录解析。
- adapter 不再直接拼接平台路径字符串作为主要实现方式。

### 8.2 会话能力建模

- 外部进程发现结果已增加 `TerminalSessionInfo { name, verified }`。
- “探测到外部进程”与“确认存在可重连终端”已经拆分建模。
- verified tmux session 才允许历史读取、输入写入与窗口 resize。
...
以下内容不再属于本 RFC 的前置阻塞项，但仍是后续可继续增强的方向：

- 更完整的 health check 与错误码体系
- 自动重启与退避策略
- 更高层的集成测试 harness
- 远程主机等新 runtime 后端
